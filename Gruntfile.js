...
module.exports = function(grunt) {
	grunt.initConfig({
		...
		sass: {
			options: {
				includePaths: ['<%= app %>/bower_components/foundation/scss','<%= app %>/bower_components/bower-bourbon']
			},
			dist: {
				options: {
		...
		watch: {
			...
			livereload: {
				files: ['<%= app %>/**/*.html', '<%= app %>/scss/**/*.scss', '!<%= app %>/bower_components/**', '<%= app %>/js/**/*.js', '<%= app %>/css/**/*.css', '<%= app %>/images/**/*.{jpg,gif,svg,jpeg,png}'],
				options: {
					livereload: true
				}
			}
